...
export async function POST(request: NextRequest) {
  try {
    ...
    const { strategyId, model, performanceGoals } =
      OptimizeStrategyRequestSchema.parse(body);
    ...
    if (!recentBacktest) {
      return NextResponse.json(
        {
          error:
            "Strategy must have at least one completed backtest before optimization",
        },
        { status: 400 },
      );
    }
    ...
    const ai = createOpenRouterAI(process.env.OPENROUTER_API_KEY, model);
    ...
    const optimizedStrategyData = await ai.optimizeStrategy(
      {
        ...
        rules: strategy.rules as any, // Cast rules to avoid type issues
        aiPrompt: strategy.aiPrompt || undefined, // Fix type compatibility
      },
      performanceData,
    );
    ...
    const optimizedStrategy = await prisma.strategy.create({
      data: {
        name:
          optimizedStrategyData.name || strategy.name || "Optimized Strategy",
        description:
          optimizedStrategyData.description || strategy.description || null,
        symbol: optimizedStrategyData.symbol || strategy.symbol || "EURUSD",
        timeframe:
          optimizedStrategyData.timeframe || strategy.timeframe || "H1",
