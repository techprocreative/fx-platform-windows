...
function validateDateRange(startDate: Date, endDate: Date): {
  ...
  const adjustedStartDate = isValidTradingDay(startDate) ? startDate : getNextTradingDay(startDate);
  const adjustedEndDate = isValidTradingDay(endDate) ? endDate : getPreviousTradingDay(endDate);
  
  // Add warnings for date adjustments
  if (adjustedStartDate.getTime() !== startDate.getTime()) {
    warnings.push(`Start date adjusted to ${adjustedStartDate.toISOString().split('T')[0]} (nearest trading day)`);
  }
  
  if (adjustedEndDate.getTime() !== endDate.getTime()) {
    warnings.push(`End date adjusted to ${adjustedEndDate.toISOString().split('T')[0]} (nearest trading day)`);
  }
...
function acquireBacktestLock(userId: string, strategyId: string): string | null {
  ...
  console.log(`Backtest lock acquired: ${lockId} for user ${userId}, strategy ${strategyId}`);
  return lockId;
}
...
function releaseBacktestLock(lockId: string): void {
  ...
  if (backtest) {
    ...
    console.log(`Backtest lock released: ${lockId} for user ${backtest.userId}, strategy ${backtest.strategyId}`);
  }
}
...
function cleanupExpiredLocks(): void {
  ...
  expiredLocks.forEach(lockId => {
    const backtest = runningBacktests.get(lockId);
    runningBacktests.delete(lockId);
    console.log(`Expired backtest lock cleaned up: ${lockId} for user ${backtest?.userId}`);
  });
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!lockId) {
      ...
      const userRunning = Array.from(runningBacktests.values())
        .filter(bt => bt.userId === session.user.id).length;
      const strategyRunning = Array.from(runningBacktests.values())
        .some(bt => bt.strategyId === validatedData.strategyId);
  ...
  try {
    ...
    if (includeStats) {
      ...
      const strategyRunning = strategyId
        ? Array.from(runningBacktests.values()).some(bt => bt.strategyId === strategyId)
        : false;
