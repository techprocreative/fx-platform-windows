...
export function AIPreviewCard({
  ...
  const takeProfitPips = Math.round(
    (strategy.parameters?.takeProfit || 0.004) * 10000,
  );
  const stopLossPips = Math.round(
    (strategy.parameters?.stopLoss || 0.002) * 10000,
  );
      ...
      <CardHeader>
        <div className="flex items-center gap-2 mb-2">
          ...
          <div>
            ...
            <CardDescription>
              Review your strategy before customizing or using it
            </CardDescription>
          </div>
        </div>
      </CardHeader>
      ...
      <CardContent className="space-y-6">
        ...
        <div className="rounded-lg border border-purple-200 bg-white p-4 space-y-3">
          ...
          <div className="flex items-center gap-4 pt-2 border-t border-neutral-100">
            ...
            <div className="flex items-center gap-1.5 text-sm">
              ...
              <span className="text-neutral-700">
                <span className="font-medium">{entryConditionsCount}</span>{" "}
                Condition{entryConditionsCount !== 1 ? "s" : ""}
              </span>
            </div>
            {firstCondition && (
              <div className="text-xs text-neutral-500">
                ({firstCondition.indicator}{" "}
                {firstCondition.operator?.replace("_", " ")}{" "}
        ...
        <div className="rounded-lg bg-purple-50 border border-purple-200 p-4">
          <p className="text-sm text-purple-900">
            <span className="font-medium">💡 Ready to use:</span> This strategy
            is complete with all required fields. Click "Use As-Is" to create
            and backtest immediately, or "Review & Customize" to fine-tune the
            parameters.
          </p>
        </div>
