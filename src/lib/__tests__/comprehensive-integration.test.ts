...
describe('Comprehensive Integration Tests', () => {
    ...
    it('should export reports in multiple formats', async () => {
      ...
      for (const format of formats) {
        ...
        if (format === 'pdf') {
          ...
          expect(reportResponse.headers['content-type']).toBe('application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        } else if (format === 'csv') {
  ...
  describe('Performance and Load Testing', () => {
    it('should handle large datasets efficiently', async () => {
      ...
      const trades = Array(1000).fill(null).map((_, index) =>
    ...
    it('should maintain performance under load', async () => {
      const loadTestRequests = Array(100).fill(null).map(() =>
        request(app)
          .get('/api/health')
          .expect(200)
      );
