...
describe("Pip Conversion Logic", () => {
  describe("getPipConfig", () => {
    ...
    it("should return correct configuration for XAGUSD", () => {
      const config = getPipConfig("XAGUSD");
      expect(config).toEqual({
        pipMultiplier: 0.001,
        minPips: 50,
        maxPips: 200,
    ...
    it("should return correct configuration for USOIL", () => {
      const config = getPipConfig("USOIL");
      expect(config).toEqual({
        pipMultiplier: 0.01,
        minPips: 20,
        maxPips: 100,
    ...
    it("should return correct configuration for UKOIL", () => {
      const config = getPipConfig("UKOIL");
      expect(config).toEqual({
        pipMultiplier: 0.01,
        minPips: 20,
        maxPips: 100,
  ...
  describe("Pip Calculations", () => {
    ...
    it("should calculate profit correctly for USDJPY sell trade", () => {
      const config = getPipConfig("USDJPY");
      const openPrice = 150.0;
      const closePrice = 149.5;
      const size = 0.01;
    ...
    it("should calculate profit correctly for XAGUSD", () => {
      const config = getPipConfig("XAGUSD");
      const openPrice = 25.0;
      const closePrice = 26.0;
      const size = 0.01;
      ...
      expect(profit).toBe(100); // 1000 pips * 0.01 * 10 = $10
    });

    it("should calculate profit correctly for USOIL", () => {
      const config = getPipConfig("USOIL");
      const openPrice = 75.0;
      const closePrice = 76.0;
      const size = 0.01;
    ...
    it("should handle loss calculations correctly", () => {
      const config = getPipConfig("EURUSD");
      const openPrice = 1.1;
      const closePrice = 1.095; // Lower price = loss for buy trade
      const size = 0.01;
