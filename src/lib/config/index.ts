...
export const API_CONFIG = {
  ...
  WEBSOCKET_URL:
    ...
    (isDevelopment ? "ws://localhost:3000" : "wss://api.nexustrade.com"),
...
export const AUTH_CONFIG = {
  ...
  SESSION_UPDATE_AGE: parseInt(
    process.env.SESSION_UPDATE_AGE || "86400000",
    10,
  ), // 1 day in ms
...
export const TRADING_CONFIG = {
  ...
  MAX_POSITION_SIZE_PERCENT: parseFloat(
    process.env.MAX_POSITION_SIZE_PERCENT || "2",
  ), // 2%
  MAX_OPEN_POSITIONS: parseInt(process.env.MAX_OPEN_POSITIONS || "5", 10),

  // Backtesting
  DEFAULT_BACKTEST_BALANCE: parseFloat(
    process.env.DEFAULT_BACKTEST_BALANCE || "10000",
  ),
...
export const UI_CONFIG = {
  ...
  // Charts
  CHART_REFRESH_INTERVAL: parseInt(
    process.env.CHART_REFRESH_INTERVAL || "5000",
    10,
  ), // 5 seconds
  ...
  // Notifications
  NOTIFICATION_DURATION: parseInt(
    process.env.NOTIFICATION_DURATION || "5000",
    10,
  ), // 5 seconds
...
export const FEATURE_FLAGS = {
  ...
  AI_STRATEGY_GENERATION:
    process.env.FEATURE_AI_STRATEGY_GENERATION !== "false",
...
export const EXTERNAL_SERVICES = {
  ...
  // Payment processing - REMOVED (Trading platform doesn't need payment)
  // Stripe configuration removed to simplify architecture
...
export const CACHE_CONFIG = {
  // Redis
  REDIS_URL:
    process.env.REDIS_URL || (isDevelopment ? "redis://localhost:6379" : ""),
  ...
  HTTP_CACHE_STALE_WHILE_REVALIDATE: parseInt(
    process.env.HTTP_CACHE_STALE_WHILE_REVALIDATE || "60",
    10,
  ), // 1 minute
} as const;
...
export const AI_CONFIG = {
  ...
  DEFAULT_MODEL: process.env.DEFAULT_AI_MODEL || 'anthropic/claude-3-haiku:beta',

  // Strategy generation
  MAX_STRATEGIES_PER_DAY: parseInt(process.env.MAX_STRATEGIES_PER_DAY || '10', 10),
  STRATEGY_GENERATION_TIMEOUT: parseInt(process.env.STRATEGY_GENERATION_TIMEOUT || '30000', 10), // 30 seconds
} as const;
...
export const SECURITY_CONFIG = {
  ...
  CORS_ORIGINS:
    ...
    (isDevelopment ? ["http://localhost:3000"] : ["https://nexustrade.com"]),
  CORS_METHODS: ["GET", "POST", "PUT", "DELETE", "OPTIONS"],
  CORS_CREDENTIALS: true,

  // Rate limiting
  RATE_LIMIT_STRICT: process.env.RATE_LIMIT_STRICT === "true",
  RATE_LIMIT_GLOBAL_MAX: parseInt(
    process.env.RATE_LIMIT_GLOBAL_MAX || "1000",
    10,
  ),
  RATE_LIMIT_GLOBAL_WINDOW: parseInt(
    process.env.RATE_LIMIT_GLOBAL_WINDOW || "60000",
    10,
  ), // 1 minute
