  ...
  Sentry.init({
    ...
    beforeSend(event: Sentry.Event) {
      ...
      if (CONFIG.isDevelopment) {
        ...
        if (event.exception?.values?.some((exc: any) => exc.type === 'ConsoleError')) {
          return null;
        }
      }
...
export function usePerformanceMeasure(name: string) {
  const [transaction, setTransaction] = React.useState<Sentry.Transaction | undefined>();
  ...
  return {
    ...
    startSpan: (spanName: string, op: string = 'react') => 
      transaction ? transaction.startChild({ op, description: spanName }) : undefined
  };
}
 ...
 */
export function categorizeError(error: Error, context?: CustomErrorContext): ErrorCategory {
  const errorMessage = error.message.toLowerCase();
  const errorStack = error.stack?.toLowerCase() || '';
  
  // Database errors
  if (errorMessage.includes('database') || errorMessage.includes('prisma') ||
      errorMessage.includes('connection') || errorStack.includes('prisma')) {
  ...
  if (errorMessage.includes('network') || errorMessage.includes('fetch') ||
      errorMessage.includes('timeout') || errorMessage.includes('external')) {
    return ErrorCategory.EXTERNAL_API;
  }
  
  // Trading errors
  if (errorMessage.includes('trade') || errorMessage.includes('order') ||
      errorMessage.includes('position') || errorMessage.includes('broker')) {
  ...
  // Backtest errors
  if (errorMessage.includes('backtest') || errorMessage.includes('strategy') ||
      errorMessage.includes('historical') || errorMessage.includes('simulation')) {
    return ErrorCategory.BACKTEST;
  }
  
  // Performance errors
  if (errorMessage.includes('timeout') || errorMessage.includes('performance') ||
      errorMessage.includes('memory') || errorMessage.includes('slow')) {
    return ErrorCategory.PERFORMANCE;
  }
  
  // System errors
  if (errorMessage.includes('system') || errorMessage.includes('server') ||
      errorMessage.includes('internal') || errorMessage.includes('crash')) {
    return ErrorCategory.SYSTEM;
  }
  
  // User interface errors
  if (errorStack.includes('react') || errorStack.includes('component') ||
      errorStack.includes('render') || context?.component) {
...
export function determineErrorSeverity(
  ...
  if (category === ErrorCategory.SYSTEM ||
      category === ErrorCategory.DATABASE && errorMessage.includes('connection') ||
      category === ErrorCategory.AUTHENTICATION && errorMessage.includes('critical') ||
      errorMessage.includes('crash') || errorMessage.includes('fatal')) {
  ...
  if (category === ErrorCategory.VALIDATION ||
      ...
      errorMessage.includes('warning') || errorMessage.includes('deprecated')) {
    return ErrorSeverity.MEDIUM;
  }
  
  // Low severity errors
  if (category === ErrorCategory.USER_INTERFACE ||
      errorMessage.includes('info') || errorMessage.includes('minor')) {
...
export class UserSessionTracker {
   ...
   */
  trackPerformance(sessionId: string, loadTime: number, success: boolean): void {
    const session = this.sessions.get(sessionId);
    if (session) {
      ...
      const total = session.performanceMetrics.averageLoadTime * (session.performanceMetrics.totalRequests - 1) + loadTime;
      session.performanceMetrics.averageLoadTime = total / session.performanceMetrics.totalRequests;
      
      Sentry.setMetrics({
        session_avg_load_time: session.performanceMetrics.averageLoadTime,
        session_success_rate: ((session.performanceMetrics.totalRequests - session.performanceMetrics.failedRequests) / session.performanceMetrics.totalRequests) * 100
      });
    }
  }
  ...
  return {
    start: () => {
      ...
      if (startTime && context?.sessionId) {
        ...
        UserSessionTracker.getInstance().trackPerformance(context.sessionId, duration, success);
      }
    },
...
export class ErrorRateMonitor {
  ...
  recordError(category: ErrorCategory, context?: CustomErrorContext): void {
    ...
    if (current && current.count > 10) { // 10 errors in 5 minutes
   ...
   */
  private triggerAlert(category: ErrorCategory, count: number, context?: CustomErrorContext): void {
    captureEnhancedError(
      new Error(`High error rate detected: ${count} errors in 5 minutes for category: ${category}`),
      {
        ...context,
        additionalData: {
          ...context?.additionalData,
