...
export const commonSchemas = {
  ...
  maxLengthString: (max: number) =>
    z.string().max(max, `Value cannot exceed ${max} characters`),
};
...
const orderStatus = z.enum(["PENDING", "EXECUTED", "CANCELLED", "REJECTED"], {
  invalid_type_error: "Invalid order status",
});

// Timeframe validation
const timeframe = z.enum(["M1", "M5", "M15", "M30", "H1", "H4", "D1", "W1"], {
