...
export function getDefaultPasswordPolicy(): PasswordPolicy {
  return {
    minLength: getEnvVar('MIN_PASSWORD_LENGTH'),
    requireUppercase: getEnvVar('PASSWORD_REQUIRE_UPPERCASE'),
    requireLowercase: getEnvVar('PASSWORD_REQUIRE_LOWERCASE'),
    requireNumbers: getEnvVar('PASSWORD_REQUIRE_NUMBERS'),
    requireSymbols: getEnvVar('PASSWORD_REQUIRE_SYMBOLS'),
    maxAge: getEnvVar('PASSWORD_MAX_AGE_DAYS'),
...
export function createPasswordSchema(policy: PasswordPolicy = getDefaultPasswordPolicy()) {
...
const commonPasswords = [
  ...
  '123123', 'qwertyuiop', 'starwars', 'football', 'whatever', 'iloveyou',
];
...
export function validatePassword(
  ...
  if (password.length < policy.minLength) {
    ...
    result.errors.push(`Password must be at least ${policy.minLength} characters`);
 ...
 */
export function generatePassword(policy: PasswordPolicy = getDefaultPasswordPolicy()): string {
  ...
  return async (req: any, res: any, next: any) => {
    try {
      ...
      if (!result.isValid) {
        ...
        }, req).catch(error => console.error('Error logging password validation failure:', error));
