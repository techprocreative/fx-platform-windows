...
export function getDefaultSecurityConfig(): SecurityHeaderConfig {
  const enableHelmet = getEnvVar('ENABLE_HELMET_MIDDLEWARE');
  const enableCSP = getEnvVar('ENABLE_CSP');
  const cspNonceGeneration = getEnvVar('CSP_NONCE_GENERATION');
...
export function getSecurityHeaders(
  ...
  if (config.enableCSP) {
    headers['Content-Security-Policy'] = buildCSPHeader(config.cspDirectives, nonce);
  }
 ...
 */
export function getAPISecurityHeaders(apiRoute: string): Record<string, string> {
...
export function validateSecurityConfig(config: SecurityHeaderConfig): boolean {
  try {
    ...
    if (config.enableCSP) {
      if (!config.cspDirectives || Object.keys(config.cspDirectives).length === 0) {
...
export function getSecurityReport(): {
  ...
  if (!config.enableHSTS && environment === 'production') {
    recommendations.push('Enable HTTP Strict Transport Security (HSTS) in production');
  }
