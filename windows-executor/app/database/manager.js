...
class DatabaseManager {
    ...
    async initialize() {
        try {
            ...
            this.db = {
                data: new Map(),
                prepare: (query) => ({
                    run: (params) => this.mockExecute(query, params),
                    get: (params) => this.mockGet(query, params),
                    all: (params) => this.mockAll(query, params),
                }),
                exec: (query) => this.mockExecute(query),
            };
            // Create tables
     ...
     * Save configuration
     */
    async saveConfig(key, value, encrypted = false) {
        try {
            let dataToStore = JSON.stringify(value);
            if (encrypted) {
                const encryptedData = crypto_1.CryptoUtils.encrypt(dataToStore, this.encryptionKey);
    ...
    async getConfig(key) {
        try {
            ...
            if (result.encrypted) {
                ...
                const decryptedData = crypto_1.CryptoUtils.decrypt(encryptedData.data, this.encryptionKey, encryptedData.iv, encryptedData.tag);
     ...
     */
    async saveRateLimitEntry(entry) {
        try {
            const stmt = this.db.prepare(`
     ...
     */
    async saveSecuritySession(session) {
        try {
            const stmt = this.db.prepare(`
     ...
     */
    async getSecuritySession(sessionId) {
        try {
