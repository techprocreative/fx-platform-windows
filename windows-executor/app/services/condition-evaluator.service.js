...
class ConditionEvaluatorService {
    ...
    async evaluateCondition(condition, marketData, currentPrice) {
        ...
        try {
            ...
            logger_1.logger.error('[ConditionEvaluator] Error evaluating condition:', error);
            return {
                conditionId: condition.id,
                indicator: condition.indicator,
                currentValue: 'ERROR',
    ...
    async getComparisonValue(value, indicatorType, params, marketData, currentPrice) {
        ...
        logger_1.logger.warn(`[ConditionEvaluator] Unknown comparison value: ${value}`);
        return 0;
    }
    /**
    ...
    performComparison(value1, value2, operator, marketData, indicator) {
        switch (operator) {
            ...
            case 'outside':
                return false;
            default:
                logger_1.logger.warn(`[ConditionEvaluator] Unknown operator: ${operator}`);
                return false;
        }
    }
    /**
