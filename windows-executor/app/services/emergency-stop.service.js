...
exports.EmergencyStopService = void 0;
const events_1 = require("events");
const logger_1 = require("../utils/logger");
class EmergencyStopService extends events_1.EventEmitter {
    constructor() {
        super();
        this.killSwitchActive = false;
        this.activationDetails = null;
        this.lockDuration = 60 * 60 * 1000; // 1 hour default
        this.lockedUntil = null;
        this.autoStopReasons = new Set();
        logger_1.logger.info('[EmergencyStop] Service initialized');
    }
    /**
    ...
    async activate(reason, triggeredBy = 'manual', severity = 'critical') {
        if (this.killSwitchActive) {
            logger_1.logger.warn('[EmergencyStop] Kill switch already active');
            return;
        }
        ...
        logger_1.logger.critical(`🔴 KILL SWITCH ACTIVATED: ${reason}`, {
            triggeredBy,
            severity,
        });
        ...
        try {
            ...
            logger_1.logger.info('[EmergencyStop] Step 1: Stopping all strategy monitors...');
            await this.stopAllStrategyMonitors();
            this.emit('killswitch:monitors_stopped');
            // STEP 2: Close all open positions
            logger_1.logger.info('[EmergencyStop] Step 2: Closing all open positions...');
            ...
            logger_1.logger.info('[EmergencyStop] Step 3: Canceling all pending orders...');
            ...
            logger_1.logger.info(`[EmergencyStop] Step 4: Trading locked for ${lockUntil / 1000 / 60} minutes`);
            // STEP 5: Notify platform
            logger_1.logger.info('[EmergencyStop] Step 5: Notifying platform...');
            ...
            logger_1.logger.info('[EmergencyStop] Step 6: Creating emergency backup...');
            await this.createEmergencyBackup();
            logger_1.logger.critical('[EmergencyStop] ✅ Kill switch activation completed successfully');
            ...
            logger_1.logger.critical('[EmergencyStop] ❌ Error during kill switch activation:', error);
    ...
    async deactivate(adminConfirmation = false) {
        if (!this.killSwitchActive) {
            logger_1.logger.warn('[EmergencyStop] Kill switch is not active');
            return;
        }
        ...
        if (this.lockedUntil && new Date() < this.lockedUntil) {
            ...
            logger_1.logger.warn(`[EmergencyStop] Admin override: unlocking ${remainingTime} minutes early`);
        }
        logger_1.logger.info('[EmergencyStop] Deactivating kill switch...');
        this.killSwitchActive = false;
        this.lockedUntil = null;
        ...
        logger_1.logger.info('[EmergencyStop] Kill switch deactivated - trading can resume');
    }
    /**
    ...
    addAutoStopReason(reason) {
        ...
        logger_1.logger.info(`[EmergencyStop] Auto-stop reason added: ${reason}`);
    }
    /**
    ...
    async lockTrading(duration) {
        ...
        logger_1.logger.warn(`[EmergencyStop] Trading locked until ${this.lockedUntil.toISOString()}`);
...
exports.EmergencyStopService = EmergencyStopService;
