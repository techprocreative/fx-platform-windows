...
exports.MTFAnalysisService = void 0;
...
class MTFAnalysisService {
     ...
     */
    async analyzeMultiTimeframe(config, symbol, timeframeData) {
        if (!config.enabled) {
            return {
                primarySignal: false,
                confirmations: [],
        ...
        if (!primaryData) {
            logger_1.logger.warn(`[MTFAnalysisService] Primary timeframe ${config.primaryTimeframe} data not found`);
            return {
                primarySignal: false,
                confirmations: [],
                overallSignal: false,
        ...
        const primarySignal = await this.evaluatePrimaryConditions(config.rules.entry.primary, primaryData, symbol);
        logger_1.logger.info(`[MTFAnalysisService] Primary signal (${config.primaryTimeframe}): ${primarySignal}`);
        ...
        for (const confirmation of config.rules.entry.confirmation) {
            ...
            if (!tfData) {
                logger_1.logger.warn(`[MTFAnalysisService] Confirmation timeframe ${confirmation.timeframe} data not found`);
                continue;
            }
            const met = await this.evaluateConfirmationCondition(confirmation.condition, tfData, symbol);
            confirmations.push({
            ...
            logger_1.logger.info(`[MTFAnalysisService] Confirmation ${confirmation.timeframe}: ${met ? 'MET' : 'NOT MET'} (required: ${confirmation.required})`);
        }
        // Determine overall signal
        let overallSignal = primarySignal;
        // All required confirmations must be met
        const requiredMet = confirmations
            .filter(c => config.rules.entry.confirmation.find(conf => conf.timeframe === c.timeframe && conf.required))
            .every(c => c.met);
        if (!requiredMet) {
            overallSignal = false;
            logger_1.logger.info('[MTFAnalysisService] Required confirmations not met');
        }
        ...
        logger_1.logger.info(`[MTFAnalysisService] Overall signal: ${overallSignal}, Confidence: ${(confidence * 100).toFixed(1)}%`);
        return {
            primarySignal,
            confirmations,
            overallSignal,
     ...
     * Evaluate primary conditions
     */
    async evaluatePrimaryConditions(conditions, timeframeData, symbol) {
        ...
        const results = await Promise.all(conditions.map(condition => this.conditionEvaluator.evaluateCondition(condition, marketData, currentPrice)));
     ...
     */
    async evaluateConfirmationCondition(condition, timeframeData, symbol) {
        const marketData = {
            symbol,
            timeframe: timeframeData.timeframe,
            bars: timeframeData.bars
     ...
     */
    getRecommendedConfirmationTimeframes(primaryTimeframe) {
     ...
     */
    async checkTimeframeAlignment(timeframeData, alignmentType) {
        let alignedCount = 0;
     ...
     */
    calculateSimpleEMA(bars, period) {
     ...
     */
    async getTimeframeTrends(timeframeData) {
        const trends = {};
        for (const tfData of timeframeData) {
            const bars = tfData.bars;
            if (bars.length < 50) {
...
exports.MTFAnalysisService = MTFAnalysisService;
