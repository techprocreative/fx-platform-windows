...
- Error rate: ${(metrics.errorRate * 100).toFixed(2)}%
    ...
    createAlert(level, category, message, value, threshold) {
        ...
        // Log based on level
        if (level === 'error') {
            logger_1.logger.error(`[PerformanceMonitor] ${message}`);
        }
        else if (level === 'warning') {
            logger_1.logger.warn(`[PerformanceMonitor] ${message}`);
        }
        else {
            logger_1.logger.info(`[PerformanceMonitor] ${message}`);
        }
    }
    /**
     * Check system health
    ...
    reset() {
        ...
        this.alertHistory = [];
        logger_1.logger.info('[PerformanceMonitor] Metrics reset');
    }
    /**
     * Set threshold
     */
    setThreshold(key, value) {
        ...
        logger_1.logger.info(`[PerformanceMonitor] Threshold ${key} set to ${value}`);
    }
    /**
     * Start periodic monitoring
     */
    startPeriodicMonitoring() {
        ...
        setInterval(() => {
            ...
            if (this.metrics.evaluationCount % 300 === 0 && this.metrics.evaluationCount > 0) {
                logger_1.logger.info('[PerformanceMonitor]', this.getSummary());
            }
        }, 60000); // Every minute
        logger_1.logger.info('[PerformanceMonitor] Periodic monitoring started');
    }
    /**
     * Get uptime
     */
