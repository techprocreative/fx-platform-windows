...
class SafetyService extends events_1.EventEmitter {
    constructor(db) {
        super();
        this.isEnabled = true;
     ...
     */
    async checkBeforeTrade(params, currentMetrics) {
        const check = {
            passed: true,
            warnings: [],
            errors: [],
        ...
        try {
            ...
            await logger_1.logger.info('Safety check completed', {
                tradeParams: params,
                result: check,
            ...
            await logger_1.logger.error('Error during safety check', { error, params });
            check.passed = false;
            check.errors.push(`Safety check error: ${error.message}`);
            return check;
        }
    }
    /**
    ...
    async emergencyStop(reason) {
        ...
        try {
            await logger_1.logger.warn('EMERGENCY STOP ACTIVATED', { reason });
            ...
            await logger_1.logger.info('Emergency stop completed', {
                reason,
            ...
            if (responseTime > 1000) {
                await logger_1.logger.warn('Emergency stop response time exceeded 1 second', { responseTime });
            }
        }
        catch (error) {
            await logger_1.logger.error('Error during emergency stop', { error, reason });
            throw error;
        }
    }
    /**
    ...
    async resetEmergencyStop() {
        try {
            this.emergencyStopActive = false;
            await logger_1.logger.info('Emergency stop reset');
            this.emit('emergencyStopReset', { timestamp: new Date() });
        }
        catch (error) {
            await logger_1.logger.error('Error resetting emergency stop', { error });
            throw error;
        }
    }
    /**
    ...
    async updateLimits(newLimits) {
        try {
            ...
            await logger_1.logger.info('Safety limits updated', { newLimits: this.limits });
            this.emit('limitsUpdated', { limits: this.limits });
        }
        catch (error) {
            await logger_1.logger.error('Error updating safety limits', { error, newLimits });
            throw error;
        }
    }
    /**
    ...
    setEnabled(enabled) {
        ...
        logger_1.logger.info(`Safety service ${enabled ? 'enabled' : 'disabled'}`);
        this.emit('safetyToggled', { enabled });
    }
    /**
     * Calculate current drawdown percentage
     */
    async calculateCurrentDrawdown(metrics) {
        try {
            ...
            await logger_1.logger.error('Error calculating drawdown', { error });
            return 0;
        }
    }
    /**
     ...
     */
    calculateRiskPercentage(params, metrics) {
        try {
            ...
            logger_1.logger.error('Error calculating risk percentage', { error, params });
            return 0;
        }
    }
    /**
    ...
    async closeAllPositions() {
        try {
            await logger_1.logger.info('Closing all positions');
            // This would integrate with the trading service
            this.emit('closeAllPositions', { timestamp: new Date() });
        }
        catch (error) {
            await logger_1.logger.error('Error closing all positions', { error });
        }
    }
    /**
    ...
    async pauseAllStrategies() {
        try {
            await logger_1.logger.info('Pausing all strategies');
            // This would integrate with the strategy management
            this.emit('pauseAllStrategies', { timestamp: new Date() });
        }
        catch (error) {
            await logger_1.logger.error('Error pausing strategies', { error });
        }
    }
    /**
     * Disconnect from MT5 (placeholder implementation)
     */
    async disconnectFromMT5() {
        try {
            await logger_1.logger.info('Disconnecting from MT5');
            // This would integrate with the MT5 service
            this.emit('disconnectFromMT5', { timestamp: new Date() });
        }
        catch (error) {
            await logger_1.logger.error('Error disconnecting from MT5', { error });
        }
    }
    /**
     * Notify admin about emergency stop (placeholder implementation)
     */
    async notifyAdminEmergencyStop(reason) {
        try {
            await logger_1.logger.info('Notifying admin about emergency stop', { reason });
            // This would send notifications via email, webhook, etc.
            this.emit('adminNotified', { reason, timestamp: new Date() });
        }
        catch (error) {
            await logger_1.logger.error('Error notifying admin', { error });
        }
    }
    /**
     * Log security event
     */
    async logSecurityEvent(type, severity, message) {
        try {
            const event = {
                id: `sec_${Date.now()}`,
                type: type,
            ...
            await logger_1.logger.error('Error logging security event', { error });
        }
    }
    /**
