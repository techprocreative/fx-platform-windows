...
class SecurityService extends events_1.EventEmitter {
    constructor(db, encryptionKey) {
        ...
        this.db = db;
        this.secureStorage = new crypto_1.SecureStorage(encryptionKey);
        this.config = this.getDefaultConfig();
    }
    getDefaultConfig() {
        return {
            encryptionKey: crypto_1.CryptoUtils.generateKey(),
    ...
    async storeCredentials(credentials) {
        try {
            ...
            const encryptedSecret = crypto_1.CryptoUtils.encrypt(credentials.apiSecret, this.config.encryptionKey);
    ...
    async getCredentials() {
        try {
            ...
            const decryptedSecret = crypto_1.CryptoUtils.decrypt(secretData.data, this.config.encryptionKey, secretData.iv, secretData.tag);
    ...
    async createSession(userId, ipAddress, userAgent) {
        try {
            const sessionId = crypto_1.CryptoUtils.generateUUID();
            const now = new Date();
            const session = {
                id: sessionId,
                userId,
    ...
    async validateApiKey(apiKey, apiSecret) {
        try {
            ...
            const apiKeyValid = crypto_1.CryptoUtils.constantTimeCompare(credentials.apiKey, apiKey);
            const apiSecretValid = crypto_1.CryptoUtils.constantTimeCompare(credentials.apiSecret, apiSecret);
    ...
    async logSecurityEvent(type, severity, message, details) {
        try {
            const event = {
                id: crypto_1.CryptoUtils.generateUUID(),
                type: type,
                timestamp: new Date(),
                severity: severity,
                message,
