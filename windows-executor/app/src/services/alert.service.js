...
class AlertService extends events_1.EventEmitter {
    ...
    acknowledgeAlert(alertId) {
        ...
        if (alert) {
            ...
            logger_1.logger.info(`[AlertService] Alert acknowledged: ${alertId}`);
    ...
    acknowledgeAll(level) {
        ...
        if (count > 0) {
            logger_1.logger.info(`[AlertService] Acknowledged ${count} alerts`);
    ...
    clearOldAlerts(olderThanMs = 86400000) {
        ...
        if (cleared > 0) {
            logger_1.logger.info(`[AlertService] Cleared ${cleared} old alerts`);
        }
        return cleared;
    }
    /**
    ...
    startPeriodicCleanup(intervalMs = 3600000) {
        ...
        logger_1.logger.info(`[AlertService] Periodic cleanup started (every ${intervalMs}ms)`);
    }
}
