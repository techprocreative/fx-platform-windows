...
class CommandService {
    constructor(zeroMQService, pusherService, safetyLimits, rateLimitConfig, logger, apiService) {
        ...
        this.commandQueue = new priority_queue_1.PriorityQueue(1000);
     ...
     */
    async executeCommand(command, queueItem) {
     ...
     */
    async executeOpenPosition(command, context) {
     ...
     */
    async executeClosePosition(command, context) {
     ...
     */
    async executeCloseAllPositions(command, context) {
     ...
     */
    async executeModifyPosition(command, context) {
        const params = command.parameters;
        if (!params?.ticket) {
     ...
     */
    async executeGetPositions(command, context) {
     ...
     */
    async executeGetAccountInfo(command, context) {
     ...
     */
    async executeGetSymbolInfo(command, context) {
    ...
    validateCommand(command) {
        ...
        switch (command.command) {
            case 'OPEN_POSITION':
                const tradeParams = command.parameters;
                if (!tradeParams?.symbol)
                    errors.push('Symbol is required for opening position');
                if (!tradeParams?.type)
                    errors.push('Trade type is required for opening position');
                if (!tradeParams?.volume || tradeParams.volume <= 0)
                    errors.push('Valid volume is required');
                break;
            case 'CLOSE_POSITION':
                if (!command.parameters?.ticket)
                    errors.push('Ticket is required for closing position');
                break;
            case 'GET_SYMBOL_INFO':
                if (!command.parameters?.symbol)
                    errors.push('Symbol is required for getting symbol info');
                break;
        }
        return {
     ...
     */
    async performSafetyCheck(command) {
        const warnings = [];
        const errors = [];
        try {
     ...
     * Create execution context
     */
    async createExecutionContext(command) {
        const positions = await this.zeroMQService.getPositions();
        return {
            command,
    ...
    getCommandPriority(command) {
        ...
        let priority = priority_queue_1.PRIORITIES[command.priority] || priority_queue_1.PRIORITIES.NORMAL;
    ...
    async reportCommandResult(commandId, result) {
        try {
            ...
            if (this.apiService) {
                await this.apiService.reportCommandResult(commandId, result.success ? 'executed' : 'failed', result);
            }
    ...
    defaultLogger(level, message, metadata) {
        ...
        console[level] ? console[level](`[${timestamp}] [${level}] [CommandService] ${message}`, metadata) : console.log(logEntry);
    }
    /**
