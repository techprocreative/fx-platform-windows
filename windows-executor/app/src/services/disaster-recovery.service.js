...
class DisasterRecoveryService extends events_1.EventEmitter {
    constructor(config) {
        super();
        ...
        logger_1.logger.info('[DisasterRecovery] Service initialized', { config: this.config });
    ...
    startAutoBackup() {
        if (!this.config.autoBackup) {
            logger_1.logger.info('[DisasterRecovery] Auto backup is disabled');
            return;
        }
        if (this.backupInterval) {
            logger_1.logger.warn('[DisasterRecovery] Auto backup already running');
            return;
        }
        logger_1.logger.info(`[DisasterRecovery] Starting auto backup (interval: ${this.config.backupInterval / 1000 / 60} minutes)`);
    ...
    stopAutoBackup() {
        if (this.backupInterval) {
            ...
            this.backupInterval = null;
            logger_1.logger.info('[DisasterRecovery] Auto backup stopped');
        }
    }
    /**
     * Perform backup
    ...
    async performBackup(type = 'manual') {
        if (this.isBackupInProgress) {
            logger_1.logger.warn('[DisasterRecovery] Backup already in progress, skipping');
        ...
        try {
            ...
            logger_1.logger.info(`[DisasterRecovery] Starting ${type} backup: ${backupId}`);
            ...
            logger_1.logger.info(`[DisasterRecovery] Backup completed: ${backupId} (${metadata.duration}ms)`);
            return backupId;
        }
        catch (error) {
            logger_1.logger.error('[DisasterRecovery] Backup failed:', error);
    ...
    async restoreFromBackup(backupId) {
        logger_1.logger.info(`[DisasterRecovery] Starting restoration from backup: ${backupId}`);
        try {
            const backupDir = path.join(this.config.backupPath, backupId);
            ...
            logger_1.logger.info('[DisasterRecovery] Backup metadata loaded:', metadata);
            // Verify integrity
            ...
            logger_1.logger.info('[DisasterRecovery] Step 1: Stopping all trading...');
            this.emit('stop-all-trading');
            await this.waitForTradingStop();
            // 2. Restore database
            logger_1.logger.info('[DisasterRecovery] Step 2: Restoring database...');
            await this.restoreDatabase(backupDir);
            // 3. Restore configurations
            logger_1.logger.info('[DisasterRecovery] Step 3: Restoring configurations...');
            await this.restoreConfigurations(backupDir);
            // 4. Restore active strategies
            logger_1.logger.info('[DisasterRecovery] Step 4: Restoring strategies...');
            await this.restoreActiveStrategies(backupDir);
            // 5. Verify restoration
            logger_1.logger.info('[DisasterRecovery] Step 5: Verifying restoration...');
            await this.verifyRestoration(metadata);
            this.emit('restoration-completed', { backupId, metadata });
            logger_1.logger.info(`[DisasterRecovery] Restoration completed successfully from ${backupId}`);
        }
        catch (error) {
            logger_1.logger.error('[DisasterRecovery] Restoration failed:', error);
    ...
    async recoverFromCrash() {
        logger_1.logger.critical('[DisasterRecovery] ⚠️ CRASH RECOVERY INITIATED');
        try {
            // 1. Check for orphaned positions
            logger_1.logger.info('[DisasterRecovery] Step 1: Checking for orphaned positions...');
            const orphanedPositions = await this.findOrphanedPositions();
            if (orphanedPositions.length > 0) {
                logger_1.logger.warn(`[DisasterRecovery] Found ${orphanedPositions.length} orphaned positions`);
                await this.handleOrphanedPositions(orphanedPositions);
            }
            else {
                logger_1.logger.info('[DisasterRecovery] No orphaned positions found');
            }
            // 2. Check database integrity
            logger_1.logger.info('[DisasterRecovery] Step 2: Checking database integrity...');
            await this.checkDatabaseIntegrity();
            // 3. Restore active strategies
            logger_1.logger.info('[DisasterRecovery] Step 3: Restoring active strategies...');
            const activeStrategies = await this.getActiveStrategiesFromDB();
            for (const strategy of activeStrategies) {
                logger_1.logger.info(`[DisasterRecovery] Restoring strategy: ${strategy.name}`);
                this.emit('restore-strategy', { strategy });
            }
            // 4. Sync with platform
            logger_1.logger.info('[DisasterRecovery] Step 4: Syncing with platform...');
            await this.syncWithPlatform();
            // 5. Verify account state
            logger_1.logger.info('[DisasterRecovery] Step 5: Verifying account state...');
            await this.verifyAccountState();
            this.emit('crash-recovery-completed');
            logger_1.logger.info('[DisasterRecovery] ✅ Crash recovery completed successfully');
        }
        catch (error) {
            logger_1.logger.critical('[DisasterRecovery] ❌ Crash recovery failed:', error);
        ...
        for (const backup of backups) {
            ...
            if (fs.existsSync(metadataPath)) {
                try {
                    ...
                    logger_1.logger.warn(`[DisasterRecovery] Invalid backup: ${backup}`, error);
                }
            }
        }
    ...
    ensureBackupDirectory() {
        if (!fs.existsSync(this.config.backupPath)) {
            ...
            logger_1.logger.info(`[DisasterRecovery] Backup directory created: ${this.config.backupPath}`);
        }
    }
    async backupDatabase(backupDir) {
        ...
        if (fs.existsSync(dbPath)) {
            ...
            logger_1.logger.debug('[DisasterRecovery] Database backed up');
        }
    }
    async backupConfigurations(backupDir) {
        ...
        logger_1.logger.debug('[DisasterRecovery] Configurations backed up');
    }
    async backupLogs(backupDir) {
        ...
        logger_1.logger.debug('[DisasterRecovery] Logs backed up');
    }
    async backupActiveStrategies(backupDir) {
        ...
        logger_1.logger.debug('[DisasterRecovery] Active strategies backed up');
    }
    async backupPerformanceMetrics(backupDir) {
        ...
        logger_1.logger.debug('[DisasterRecovery] Performance metrics backed up');
    }
    async restoreDatabase(backupDir) {
        ...
        if (fs.existsSync(backupDbPath)) {
            ...
            logger_1.logger.info('[DisasterRecovery] Database restored');
        }
    }
    async restoreConfigurations(backupDir) {
        ...
        if (fs.existsSync(configDir)) {
            ...
            logger_1.logger.info('[DisasterRecovery] Configurations restored');
        }
    }
    async restoreActiveStrategies(backupDir) {
        ...
        if (fs.existsSync(strategiesPath)) {
            ...
            logger_1.logger.info(`[DisasterRecovery] ${strategies.length} strategies restored`);
        }
    }
        ...
        if (recoveryPoints.length > this.config.maxBackups) {
            ...
            for (const point of toDelete) {
                try {
                    ...
                    logger_1.logger.debug(`[DisasterRecovery] Deleted old backup: ${point.id}`);
                }
                catch (error) {
                    logger_1.logger.warn(`[DisasterRecovery] Failed to delete backup ${point.id}:`, error);
                }
            }
            logger_1.logger.info(`[DisasterRecovery] Cleaned up ${toDelete.length} old backups`);
        }
    }
    async uploadToCloud(backupDir) {
        // TODO: Implement cloud upload
        logger_1.logger.debug('[DisasterRecovery] Cloud upload not implemented yet');
    }
    setupEventTriggers() {
    ...
    async verifyRestoration(metadata) {
        ...
        logger_1.logger.info('[DisasterRecovery] Verifying restoration...');
        // Check database
        if (!fs.existsSync('./executor.db')) {
            throw new Error('Database not found after restoration');
        }
        logger_1.logger.info('[DisasterRecovery] Restoration verification passed');
    }
    ...
    async handleOrphanedPositions(positions) {
        logger_1.logger.warn(`[DisasterRecovery] Handling ${positions.length} orphaned positions`);
        // TODO: Sync orphaned positions with database
    }
    async checkDatabaseIntegrity() {
        // TODO: Run database integrity checks
        logger_1.logger.info('[DisasterRecovery] Database integrity check passed');
    }
    ...
    async syncWithPlatform() {
        ...
        logger_1.logger.info('[DisasterRecovery] Synced with platform');
    }
    async verifyAccountState() {
        // TODO: Verify MT5 account state matches database
        logger_1.logger.info('[DisasterRecovery] Account state verified');
    }
}
