...
exports.AdvancedIndicatorService = void 0;
...
class AdvancedIndicatorService {
    ...
    calculateBollingerBands(data, period = 20, stdDev = 2) {
        logger_1.logger.debug(`[AdvancedIndicator] Calculating Bollinger Bands (period: ${period}, stdDev: ${stdDev})`);
    ...
    calculateStochastic(data, kPeriod = 14, dPeriod = 3, smooth = 3) {
        logger_1.logger.debug(`[AdvancedIndicator] Calculating Stochastic (K: ${kPeriod}, D: ${dPeriod})`);
        const k = [];
        const d = [];
        const signal = [];
        // Calculate raw %K
    ...
    calculateADX(data, period = 14) {
        logger_1.logger.debug(`[AdvancedIndicator] Calculating ADX (period: ${period})`);
        const plusDM = [];
        const minusDM = [];
        const tr = [];
    ...
    calculateCCI(data, period = 20, constant = 0.015) {
        logger_1.logger.debug(`[AdvancedIndicator] Calculating CCI (period: ${period})`);
    ...
    calculateWilliamsR(data, period = 14) {
        logger_1.logger.debug(`[AdvancedIndicator] Calculating Williams %R (period: ${period})`);
        const wr = [];
        const signal = [];
        for (let i = 0; i < data.length; i++) {
            if (i < period - 1) {
    ...
    calculateIchimoku(data, tenkanPeriod = 9, kijunPeriod = 26, senkouBPeriod = 52) {
        logger_1.logger.debug(`[AdvancedIndicator] Calculating Ichimoku Cloud`);
     ...
     */
    calculateVWAP(data) {
        logger_1.logger.debug(`[AdvancedIndicator] Calculating VWAP`);
        const vwap = [];
        const deviation = [];
        let cumulativeTPV = 0;
     ...
     */
    calculateOBV(data) {
        logger_1.logger.debug(`[AdvancedIndicator] Calculating OBV`);
        const obv = [];
        const signal = [];
        let cumulativeOBV = 0;
        for (let i = 0; i < data.length; i++) {
    ...
    calculateVolumeMA(data, period = 20) {
        logger_1.logger.debug(`[AdvancedIndicator] Calculating Volume MA (period: ${period})`);
...
exports.AdvancedIndicatorService = AdvancedIndicatorService;
