...
exports.MT5DetectorService = void 0;
...
class MT5DetectorService {
    ...
    async detectAllInstallations() {
        ...
        try {
            ...
            for (const basePath of standardPaths) {
                const installation = await this.checkInstallationPath(basePath, mt5_types_1.MT5InstallationType.STANDARD);
                if (installation) {
            ...
            for (const regPath of registryPaths) {
                const installation = await this.checkInstallationPath(regPath, mt5_types_1.MT5InstallationType.BROKER_SPECIFIC);
                if (installation) {
                    installations.push(installation);
                }
            }
            ...
            for (const portablePath of portablePaths) {
                const installation = await this.checkInstallationPath(portablePath, mt5_types_1.MT5InstallationType.PORTABLE);
                if (installation) {
                    installations.push(installation);
                }
            }
            ...
            for (const process of runningProcesses) {
                const installation = await this.checkInstallationPath(process.path, mt5_types_1.MT5InstallationType.CUSTOM);
                if (installation) {
                    installations.push(installation);
                }
            }
    ...
    async getPortableInstallationPaths() {
        ...
        try {
            ...
            if (await file_utils_1.FileUtils.pathExists(terminalPath)) {
                const terminals = await file_utils_1.FileUtils.listFiles(terminalPath);
                for (const terminal of terminals) {
                    const terminalDataPath = path.join(terminalPath, terminal);
                    const originPath = path.join(terminalDataPath, "origin.txt");
                    if (await file_utils_1.FileUtils.pathExists(originPath)) {
                        try {
                            const origin = await file_utils_1.FileUtils.readFile(originPath);
                            const originPathTrimmed = origin.trim();
                            if (originPathTrimmed &&
                                (await file_utils_1.FileUtils.pathExists(originPathTrimmed))) {
    ...
    async getRunningMT5Processes() {
        ...
        try {
            ...
            try {
                const runningProcesses = await file_utils_1.FileUtils.getRunningProcesses("terminal");
    ...
    async checkInstallationPath(basePath, installationType) {
        try {
            if (!(await file_utils_1.FileUtils.pathExists(basePath))) {
                return null;
            }
            ...
            let terminalPath = "";
            let is64Bit = false;
            if (await file_utils_1.FileUtils.pathExists(terminal64)) {
                terminalPath = terminal64;
                is64Bit = true;
            }
            else if (await file_utils_1.FileUtils.pathExists(terminal32)) {
    ...
    async getMT5Info(basePath, terminalPath, is64Bit, installationType) {
        ...
        const version = await file_utils_1.FileUtils.getFileVersion(terminalPath);
        const productVersion = await file_utils_1.FileUtils.getProductVersion(terminalPath);
        ...
        await file_utils_1.FileUtils.ensureDirectory(libraryPath);
        await file_utils_1.FileUtils.ensureDirectory(expertsPath);
        // Check if MT5 is currently running
        const isRunning = await file_utils_1.FileUtils.isProcessRunning(is64Bit ? "terminal64.exe" : "terminal.exe");
    ...
    async determineDataPath(basePath, installationType) {
        ...
        if (await file_utils_1.FileUtils.pathExists(portableMQL5)) {
        ...
        if (await file_utils_1.FileUtils.pathExists(terminalDataPath)) {
            try {
                const terminals = await file_utils_1.FileUtils.listFiles(terminalDataPath);
                for (const terminal of terminals) {
                    ...
                    if (await file_utils_1.FileUtils.pathExists(mql5Path)) {
                        ...
                        if (await file_utils_1.FileUtils.pathExists(commonFilePath)) {
                            try {
                                const commonContent = await file_utils_1.FileUtils.readFile(commonFilePath);
        ...
        if (commonDataPath) {
            ...
            if (await file_utils_1.FileUtils.pathExists(metaQuotesCommon)) {
                ...
                try {
                    const terminals = await file_utils_1.FileUtils.listFiles(commonParent);
                    for (const terminal of terminals) {
                        if (terminal !== "Common") {
    ...
    async detectBroker(dataPath) {
        try {
            ...
            for (const configFile of configFiles) {
                if (await file_utils_1.FileUtils.pathExists(configFile)) {
                    try {
                        const content = await file_utils_1.FileUtils.readFile(configFile);
    ...
    async detectAccountNumber(dataPath) {
        try {
            ...
            for (const accountFile of accountFiles) {
                if (await file_utils_1.FileUtils.pathExists(accountFile)) {
                    try {
                        const content = await file_utils_1.FileUtils.readFile(accountFile);
    ...
    async getInstallationByPath(path) {
        try {
            const installation = await this.checkInstallationPath(path, mt5_types_1.MT5InstallationType.CUSTOM);
            return installation;
        }
        catch (error) {
    ...
    async validateInstallation(mt5Info) {
        try {
            ...
            const hasTerminal = (await file_utils_1.FileUtils.pathExists(terminal64)) ||
                (await file_utils_1.FileUtils.pathExists(terminal32));
            if (!hasTerminal) {
                return false;
            }
            // Check if MQL5 directories exist
            const hasLibraries = await file_utils_1.FileUtils.pathExists(mt5Info.libraryPath);
            const hasExperts = await file_utils_1.FileUtils.pathExists(mt5Info.expertsPath);
...
exports.MT5DetectorService = MT5DetectorService;
