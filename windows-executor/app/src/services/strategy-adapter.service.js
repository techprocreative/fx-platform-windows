...
exports.StrategyAdapterService = void 0;
const logger_1 = require("../utils/logger");
class StrategyAdapterService {
    /**
     * Convert database strategy format to executor format
     */
    static convertStrategy(dbStrategy) {
        try {
            logger_1.logger.info(`[StrategyAdapter] Converting strategy: ${dbStrategy.name}`);
            // Extract rules
            ...
            const exitLogic = (exit.logic || 'OR').toUpperCase();
            // Build executor-compatible strategy
            const executorStrategy = {
                id: dbStrategy.id,
            ...
            logger_1.logger.info(`[StrategyAdapter] ✅ Converted: ${executorStrategy.name}`);
            logger_1.logger.debug(`[StrategyAdapter] Entry conditions: ${entryConditions.length}, Logic: ${entryLogic}`);
            return executorStrategy;
        }
        catch (error) {
            logger_1.logger.error(`[StrategyAdapter] ❌ Failed to convert strategy:`, error);
            logger_1.logger.error(`[StrategyAdapter] Strategy data:`, JSON.stringify(dbStrategy, null, 2));
            return null;
        }
    }
    /**
     ...
     */
    static convertConditions(dbConditions) {
     ...
     */
    static convertIndicatorName(dbIndicator) {
        const mapping = {
            'rsi': 'RSI',
            'macd': 'MACD',
     ...
     */
    static extractIndicatorParams(cond) {
        const params = {};
     ...
     */
    static convertComparison(dbComparison) {
        const mapping = {
     ...
     */
    static convertTimeframe(dbTimeframe) {
        ...
        logger_1.logger.warn(`[StrategyAdapter] Invalid timeframe: ${dbTimeframe}, defaulting to M15`);
        return 'M15';
    }
    /**
     * Convert stop loss configuration
     */
    static convertStopLoss(dbStopLoss) {
        if (!dbStopLoss)
            return undefined;
        return {
     ...
     */
    static convertTakeProfit(dbTakeProfit) {
        if (!dbTakeProfit)
            return undefined;
        return {
    ...
    static validateStrategy(strategy) {
        ...
        if (!strategy.id)
            errors.push('Missing strategy ID');
        if (!strategy.name)
            errors.push('Missing strategy name');
        if (!strategy.symbols || strategy.symbols.length === 0)
            errors.push('Missing symbols');
        ...
        for (const cond of strategy.entryConditions || []) {
            if (!cond.indicator)
                errors.push(`Condition missing indicator`);
            if (!cond.comparison)
                errors.push(`Condition missing comparison`);
...
exports.StrategyAdapterService = StrategyAdapterService;
