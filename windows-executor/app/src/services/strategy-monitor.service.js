...
exports.StrategyMonitor = void 0;
const events_1 = require("events");
const logger_1 = require("../utils/logger");
const strategy_helpers_1 = require("../utils/strategy-helpers");
class StrategyMonitor extends events_1.EventEmitter {
    ...
    async startMonitoring(strategy) {
        if (this.activeMonitors.has(strategy.id)) {
            logger_1.logger.warn(`[StrategyMonitor] Strategy ${strategy.id} already being monitored`);
            return;
        }
        logger_1.logger.info(`[StrategyMonitor] Starting monitoring for strategy: ${strategy.name}`);
        const monitor = {
    ...
    async stopMonitoring(strategyId) {
        ...
        if (!monitor) {
            logger_1.logger.warn(`[StrategyMonitor] Strategy ${strategyId} not being monitored`);
            return;
        }
        logger_1.logger.info(`[StrategyMonitor] Stopping monitoring for strategy: ${monitor.strategy.name}`);
     ...
     */
    async stopAll() {
        logger_1.logger.info('[StrategyMonitor] Stopping all strategy monitors...');
        const strategyIds = Array.from(this.activeMonitors.keys());
        for (const strategyId of strategyIds) {
            await this.stopMonitoring(strategyId);
        }
        logger_1.logger.info('[StrategyMonitor] All monitors stopped');
    }
    /**
    ...
    async runMonitoringLoop(monitor) {
        ...
        logger_1.logger.info(`[StrategyMonitor] Monitor loop started for ${strategy.name} (checking every ${interval}ms)`);
        const executeLoop = async () => {
            ...
            try {
                ...
                const primarySymbol = (0, strategy_helpers_1.getPrimarySymbol)(strategy);
                ...
                if (!marketData || marketData.length === 0) {
                    logger_1.logger.warn(`[StrategyMonitor] No market data for ${primarySymbol}`);
                ...
                if (!filtersPassed) {
                    logger_1.logger.debug(`[StrategyMonitor] Filters not passed for ${strategy.name}`);
                    this.scheduleNext(monitor, interval);
                    return;
                }
                ...
                monitor.errorCount++;
                logger_1.logger.error(`[StrategyMonitor] Error in monitoring loop for ${strategy.name}:`, error);
                // Stop monitoring if too many errors
                if (monitor.errorCount > 10) {
                    logger_1.logger.critical(`[StrategyMonitor] Too many errors, stopping monitor for ${strategy.name}`);
    ...
    async evaluateEntryConditions(strategy, marketData) {
        try {
            ...
            logger_1.logger.error('[StrategyMonitor] Error evaluating entry conditions:', error);
            return null;
        }
    }
    /**
    ...
    async calculatePositionSize(strategy, signal) {
        try {
            ...
            logger_1.logger.error('[StrategyMonitor] Error calculating position size:', error);
...
exports.StrategyMonitor = StrategyMonitor;
