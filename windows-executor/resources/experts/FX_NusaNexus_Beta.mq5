...
//| Beta Version with Shared Secret Authentication                   |
//| Port 5555: Send to Executor | Port 5556: Receive Commands       |
//+------------------------------------------------------------------+
#property copyright "NusaNexus Trading Systems - Beta"
...
{
   Print("=================================================================");
   Print("FX NusaNexus Bridge - BETA VERSION with Authentication");
   Print("=================================================================");
   
   // Check if shared secret is configured
   if(StringLen(InpSharedSecret) == 0)
   {
      Alert("⚠️ WARNING: No shared secret configured!");
      Print("⚠️ EA will work but commands will NOT be authenticated!");
      Print("⚠️ Please configure shared secret from Windows Executor");
   }
   else
   {
      Print("✅ Shared secret configured: ", StringLen(InpSharedSecret), " characters");
   }
...
bool ValidateAuth(string request)
{
   ...
   if(token == "")
   {
      Print("⚠️ No authentication token in request");
      g_failedAuthCount++;
      g_lastAuthFailure = TimeCurrent();
      return false;
   }
   
   // Validate token matches shared secret
...
string ProcessCommand(string request)
{
   ...
   if(StringFind(request, "\"command\":\"PING\"") >= 0)
   {
      ...
      return "{\"status\":\"OK\",\"message\":\"PONG\",\"executionTime\":" + IntegerToString((long)executionTime) + ",\"authEnabled\":" + (StringLen(InpSharedSecret) > 0 ? "true" : "false") + "}";
   }
...
//+------------------------------------------------------------------+
//| NOTE: Add all other functions from FX_NusaNexus.mq5:            |
//| - GetHistoricalBars, GetCurrentPrice, ExecuteOpenPosition, etc. |
//| (Functions remain unchanged from original)                       |
//+------------------------------------------------------------------+
