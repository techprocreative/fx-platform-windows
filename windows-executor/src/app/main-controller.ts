...
export class MainController extends EventEmitter {
  ...
  private positionRegistry: any; // PositionRegistry
  ...
  private eaAttachmentHandler!: EAAttachmentHandler;
  ...
  private initializeServices(): void {
    ...
    const loggerFunc = (level: string, message: string, metadata?: any) => {
    ...
    this.apiService = new ApiService(loggerFunc);
    ...
    this.pusherService = new PusherService(loggerFunc);
    this.zeromqService = new ZeroMQService(loggerFunc);
    this.zeromqServer = new ZeroMQServerService(loggerFunc);
  ...
  async initialize(config: AppConfig): Promise<boolean> {
    try {
      ...
      this.mt5AccountService = new MT5AccountService(this.zeromqServer, this.zeromqService);
      ...
      logger.info('[MainController] ✅ Live trading event handlers setup complete (BEFORE monitoring starts)', { category: 'MAIN' });
          ...
          if (persistedStrategies.length > 0) {
            ...
            for (const strategy of persistedStrategies) {
              ...
              if (this.connectionStatus.zeromq === 'connected' && this.strategyMonitor) {
                await this.strategyMonitor.startMonitoring(strategy as any);
              }
            }
          }
        } catch (fallbackError) {
   ...
   */
  private async restoreEAAttachment(strategy: any): Promise<void> {
    try {
      ...
      const attachment = this.persistence.getEAAttachment(
        strategy.symbol, 
        strategy.timeframe,
        accountNumber
      );
   ...
   */
  notifyEAAttached(info: { symbol: string; timeframe: string; accountNumber: string; chartId?: string }): void {
    try {
      this.eaAttachmentHandler.notifyEAAttached(info);
      this.addLog('info', 'EA', `EA attached: ${info.symbol} ${info.timeframe}`);
    } catch (error) {
      this.addLog('error', 'EA', `Failed to track EA attachment: ${(error as Error).message}`);
    }
  }
   ...
   */
  notifyEADetached(info: { symbol: string; timeframe: string; accountNumber: string }): void {
    try {
      this.eaAttachmentHandler.notifyEADetached(info);
      this.addLog('info', 'EA', `EA detached: ${info.symbol} ${info.timeframe}`);
    } catch (error) {
      this.addLog('error', 'EA', `Failed to track EA detachment: ${(error as Error).message}`);
    }
  }
   ...
   */
  async reconnectZeroMQClient(): Promise<boolean> {
    try {
      ...
      // Reconnect
      const connected = await this.zeromqService.connect(this.config as any);
      ...
      this.addLog('error', 'ZEROMQ', `Reconnection error: ${(error as Error).message}`, { error });
      return false;
    }
  }
  ...
  private async startLiveTradingServices(): Promise<void> {
    try {
      ...
      this.commandProcessor.initialize(
        ...
        this.emergencyStop,
        this.positionRegistry  // Add position registry
      );
      ...
      setInterval(async () => {
        if (this.positionRegistry) {
          try {
            ...
            logger.error('[MainController] Position update broadcast failed', error);
          }
        }
  ...
  private setupLiveTradingEventHandlers(): void {
    logger.info('[MainController] Setting up live trading event handlers...', { category: 'MAIN' });
    
    // Strategy Monitor events (Phase 1)
    this.strategyMonitor.on('signal:generated', async (data: any) => {
      logger.info('[MainController] 🔔 SIGNAL EVENT TRIGGERED!', { category: 'SIGNAL', data });
      ...
      logger.info('[MainController] Calling handleSignalGenerated...', { category: 'SIGNAL' });
  ...
  private async handleSignalGenerated(data: any): Promise<void> {
    logger.info('[MainController] 🚀 handleSignalGenerated CALLED', { category: 'SIGNAL', data });
    const { strategyId, signal } = data;
    
    try {
      logger.info('[MainController] Processing signal...', { category: 'SIGNAL', strategyId, signal });
      ...
      logger.info('[MainController] Checking emergency stop...', { category: 'SIGNAL', canTrade: this.emergencyStop?.canTrade() });
      if (this.emergencyStop && !this.emergencyStop.canTrade()) {
        logger.warn('[MainController] ⛔ Trade rejected: Emergency stop active', { category: 'SIGNAL' });
        this.addLog('warn', 'SIGNAL', 'Trade rejected: Emergency stop is active');
        return;
      }

      // Validate signal has required fields
      logger.info('[MainController] Validating signal fields...', { category: 'SIGNAL', symbol: signal.symbol, type: signal.type, volume: signal.volume });
      if (!signal.symbol || !signal.type || !signal.volume) {
        logger.error('[MainController] ❌ Invalid signal: Missing required fields', { category: 'SIGNAL', signal });
        this.addLog('error', 'SIGNAL', 'Invalid signal: Missing required fields', { signal });
        return;
      }
      
      logger.info('[MainController] ✅ Signal validation passed!', { category: 'SIGNAL' });

      // Prepare trade parameters
      logger.info('[MainController] Preparing trade parameters...', { category: 'SIGNAL' });
      ...
      if (stopLoss === 0 && signal.entryPrice) {
        ...
        logger.info(`[MainController] 🛡️ Calculated Stop Loss: ${stopLoss.toFixed(2)} (2% from entry)`, { category: 'SIGNAL' });
      }
      ...
      if (takeProfit === 0 && signal.entryPrice && stopLoss > 0) {
        ...
        logger.info(`[MainController] 🎯 Calculated Take Profit: ${takeProfit.toFixed(2)} (2:1 R:R)`, { category: 'SIGNAL' });
      }
      ...
      logger.info('[MainController] 🎯 Executing trade via ZeroMQ...', { category: 'SIGNAL', tradeParams });
      this.addLog('info', 'SIGNAL', 'Executing trade via ZeroMQ...', { tradeParams });

      // Execute trade via ZeroMQ
      logger.info('[MainController] Calling zeromqService.openPosition...', { category: 'SIGNAL' });
      const result = await this.zeromqService.openPosition(tradeParams);
      logger.info('[MainController] 📥 Received result from openPosition', { category: 'SIGNAL', result });

      if (result.success) {
        logger.info(`[MainController] 🎉 Trade executed successfully!`, { 
          category: 'SIGNAL',
        ...
        try {
          ...
          logger.info('[MainController] 📡 Trade broadcasted to dashboard', { category: 'SIGNAL' });
        } catch (broadcastError) {
          logger.error('[MainController] Failed to broadcast trade:', broadcastError);
        }
   ...
   */
  private generateMagicNumber(strategyId: string): number {
   ...
   */
  private async reportTradeExecution(strategyId: string, signal: any, result: any): Promise<void> {
    try {
      if (!this.apiService) {
