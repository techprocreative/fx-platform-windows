...
import { Bar } from './indicator.service';
...
export class CorrelationService {
  ...
  async calculateCorrelationMatrix(
    ...
    if (cached) {
      logger.info('[CorrelationService] Using cached correlation matrix');
      return cached;
    }
    ...
    for (let i = 0; i < pairsData.length; i++) {
      ...
      for (let j = 0; j < pairsData.length; j++) {
        ...
        const corrData = this.calculatePearsonCorrelation(
          pair1.bars,
          pair2.bars,
          config.lookbackPeriod || 100
        );
  ...
  async analyzeCorrelationRisk(
    ...
    const conflictingPositions: Array<{
      symbol: string;
      correlation: number;
      positionSize: number;
    }> = [];
   ...
   */
  groupByCurrency(symbols: string[]): Record<string, string[]> {
   ...
   * Cache management
   */
  private getCacheKey(symbols: string[], config: CorrelationFilter): string {
    return `${symbols.sort().join('_')}_${config.lookbackPeriod || 100}`;
  }

  private getFromCache(key: string): CorrelationMatrix | null {
  ...
  private saveToCache(key: string, matrix: CorrelationMatrix): void {
  ...
  clearCache(): void {
    ...
    logger.info('[CorrelationService] Cache cleared');
  }
}
