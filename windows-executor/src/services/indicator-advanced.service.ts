...
export interface ADX {
  ...
  plusDI: number[];
  minusDI: number[];
  trend: ('strong_uptrend' | 'strong_downtrend' | 'weak' | 'no_trend')[];
}
...
export class AdvancedIndicatorService {
  ...
  calculateADX(
    ...
    const trend: ('strong_uptrend' | 'strong_downtrend' | 'weak' | 'no_trend')[] = [];
    ...
    const padded = {
      ...
      trend: [...Array(padLength).fill('no_trend' as const), ...trend],
    };
   ...
   */
  calculateOBV(data: OHLCV[]): { obv: number[]; signal: ('bullish' | 'bearish' | 'neutral')[] } {
  ...
  calculateVolumeMA(
    ...
    const signal: ('high_volume' | 'low_volume' | 'normal')[] = [];
  ...
  private wilderSmoothing(data: number[], period: number): number[] {
  ...
  private calculateMidpoint(data: OHLCV[], period: number): number[] {
