...
import { InstallerStep, } from "../types/mt5.types";
...
export class MT5AutoInstaller {
    ...
    async autoInstallEverything() {
        ...
        try {
            ...
            this.reportProgress(InstallerStep.DETECTING_MT5, `Found ${installations.length} MT5 installation(s)`, 20);
            ...
            for (let i = 0; i < installations.length; i++) {
                ...
                this.reportProgress(InstallerStep.INSTALLING_LIBZMQ, `Installing libzmq.dll to: ${mt5.path}`, progressBase);
                // Install libzmq.dll
                ...
                this.reportProgress(InstallerStep.INSTALLING_EXPERT_ADVISOR, `Installing Expert Advisor to: ${mt5.path}`, progressBase + 20);
                ...
                this.reportProgress(InstallerStep.CREATING_CONFIG, `Creating configuration file for: ${mt5.path}`, progressBase + 40);
            ...
            if (this.config.autoAttachEA && installations[0].isRunning) {
                this.reportProgress(InstallerStep.AUTO_ATTACHING_EA, "Auto-attaching EA to chart...", 85);
            ...
            if (this.config.verifyInstallation) {
                this.reportProgress(InstallerStep.COMPLETED, "Verifying installation...", 90);
            ...
            this.reportProgress(result.success ? InstallerStep.COMPLETED : InstallerStep.FAILED, result.success
            ...
            this.reportProgress(InstallerStep.FAILED, `✗ Installation failed: ${errorMessage}`, 100);
        }
        return result;
    }
    /**
    ...
    async installLibZMQ(mt5) {
        ...
        try {
            ...
            const copyResult = await FileUtils.copyWithBackup(libzmqSource, result.destinationPath, this.config.createBackups);
            ...
            if (result.success) {
                ...
                if (libsodiumExists) {
                    ...
                    const sodiumResult = await FileUtils.copyWithBackup(libsodiumSource, libsodiumDest, this.config.createBackups);
    ...
    async installExpertAdvisor(mt5) {
        ...
        try {
            ...
            // Copy with backup
            const copyResult = await FileUtils.copyWithBackup(eaSource, result.destinationPath, this.config.createBackups);
            result.success = copyResult.success;
            result.backupPath = copyResult.backupPath;
            result.error = copyResult.error;
            if (result.success) {
