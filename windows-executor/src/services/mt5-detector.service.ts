...
export class MT5DetectorService {
  ...
  async detectAllInstallations(): Promise<MT5Info[]> {
    ...
    try {
      ...
      for (const basePath of standardPaths) {
        const installation = await this.checkInstallationPath(
          basePath,
          MT5InstallationType.STANDARD,
        );
        if (installation) {
      ...
      for (const regPath of registryPaths) {
        const installation = await this.checkInstallationPath(
          regPath,
          MT5InstallationType.BROKER_SPECIFIC,
        );
        if (installation) {
          installations.push(installation);
        }
  ...
  private getStandardInstallationPaths(): string[] {
    ...
    if (process.env.USERPROFILE) {
      ...
      paths.push(
        path.join(process.env.USERPROFILE, "Documents", "MetaTrader 5"),
      );
      paths.push(
        path.join(process.env.USERPROFILE, "Downloads", "MetaTrader 5"),
      );
    }
        ...
        for (const terminal of terminals) {
          ...
          if (await FileUtils.pathExists(originPath)) {
            try {
              ...
              console.warn(
                `Could not read origin file for ${terminal}:`,
                error,
              );
            }
          }
    ...
    try {
      ...
      try {
        const runningProcesses =
          await FileUtils.getRunningProcesses("terminal");
        ...
        for (const proc of runningProcesses) {
          if (proc.name.includes("terminal") && proc.path) {
            const processInfo: MT5ProcessInfo = {
              ...
              isPortable:
                proc.path.includes("AppData") || proc.path.includes("portable"),
            };
      ...
      try {
        ...
        for (const exePath of processPaths) {
          const processInfo: MT5ProcessInfo = {
            ...
            isPortable:
              exePath.includes("AppData") || exePath.includes("portable"),
          };
          processes.push(processInfo);
        }
      } catch (error) {
      ...
      try {
        ...
        if (stdout.includes("terminal64.exe")) {
          ...
          console.log(
            "MT5 terminal64.exe is running (path detection via tasklist limited)",
          );
        }
      } catch (error) {
  ...
  private async checkInstallationPath(
    ...
    try {
      ...
      // Get MT5 information
      const mt5Info = await this.getMT5Info(
        basePath,
        terminalPath,
        is64Bit,
        installationType,
  ...
  private async getMT5Info(
    ...
    const build =
      this.extractBuildNumber(version) ||
    ...
    const isRunning = await FileUtils.isProcessRunning(
      is64Bit ? "terminal64.exe" : "terminal.exe",
    );
   ...
   */
  private async determineDataPath(
    basePath: string,
    installationType: MT5InstallationType,
  ): Promise<string> {
      ...
      try {
        ...
        for (const terminal of terminals) {
          ...
          if (await FileUtils.pathExists(originPath)) {
            try {
              const origin = await FileUtils.readFile(originPath);
              const normalizedOrigin = path.normalize(
          ...
          if (await FileUtils.pathExists(mql5Path)) {
            ...
            if (await FileUtils.pathExists(commonFilePath)) {
              try {
                ...
                if (
                  commonContent.toLowerCase().includes(basePath.toLowerCase())
                ) {
    ...
    console.warn(
      `Could not determine data path for ${basePath}, using installation directory`,
    );
    return basePath;
  }
   ...
   */
  private async detectAccountNumber(
    dataPath: string,
  ...
  async validateInstallation(mt5Info: MT5Info): Promise<boolean> {
    try {
      ...
      const hasTerminal =
        (await FileUtils.pathExists(terminal64)) ||
