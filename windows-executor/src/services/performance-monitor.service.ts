...
export interface PerformanceMetrics {
  ...
  indicatorCacheHitRate: number;
  averageIndicatorTime: number;
...
export class PerformanceMonitorService extends EventEmitter {
   ...
   */
  recordEvaluationTime(timeMs: number): void {
    ...
    if (timeMs > this.thresholds.slowEvaluationMs) {
      this.createAlert('warning', 'evaluation', 
        `Slow evaluation detected: ${timeMs.toFixed(2)}ms`,
        timeMs,
        this.thresholds.slowEvaluationMs
      );
    }
  }
   ...
   */
  recordIndicatorTime(timeMs: number): void {
