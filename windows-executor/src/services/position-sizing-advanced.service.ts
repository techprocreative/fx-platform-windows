...
export class AdvancedPositionSizingService {
  private readonly DEFAULT_RISK_PERCENTAGE = 1; // 1%
  private readonly DEFAULT_ATR_MULTIPLIER = 1.5;
  private readonly KELLY_FRACTION = 0.25; // Use 25% of Kelly for safety
  private readonly DEFAULT_MAX_LOT = 10.0;
  private readonly DEFAULT_MIN_LOT = 0.01;

  /**
   * Calculate position size based on method
   */
  async calculatePositionSize(params: PositionSizingParams): Promise<PositionSizeResult> {
    logger.info(`[PositionSizing] Calculating position size using ${params.method}`);
    ...
    try {
      ...
      logger.info(`[PositionSizing] Calculated lot size: ${lotSize} (risk: ${riskAmount})`);
      ...
      logger.error(`[PositionSizing] Error calculating position size:`, error);
      throw error;
    }
  }
   ...
   * Fixed lot sizing
   */
  private fixedLotSizing(params: PositionSizingParams): {
    lotSize: number;
    riskAmount: number;
  } {
   ...
   */
  private percentageRiskSizing(params: PositionSizingParams): {
    lotSize: number;
    riskAmount: number;
    confidence: number;
    warnings: string[];
   ...
   */
  private atrBasedSizing(params: PositionSizingParams): {
    lotSize: number;
    riskAmount: number;
    confidence: number;
    warnings: string[];
   ...
   */
  private kellyCriterionSizing(params: PositionSizingParams): {
    lotSize: number;
    riskAmount: number;
    confidence: number;
    warnings: string[];
   ...
   */
  private volatilityBasedSizing(params: PositionSizingParams): {
    lotSize: number;
    riskAmount: number;
    confidence: number;
    warnings: string[];
   ...
   */
  private accountEquitySizing(params: PositionSizingParams): {
    lotSize: number;
    riskAmount: number;
    confidence: number;
  } {
   ...
   */
  private calculateRiskAmount(symbol: string, lotSize: number, stopLossPips: number): number {
   ...
   */
  calculateLiveAccountLot(baseLotSize: number, accountType: 'demo' | 'live'): number {
    if (accountType === 'live') {
