...
import { IndicatorService, Bar } from './indicator.service';
...
export class PositionSizingService {
    ...
    try {
      ...
      switch (config.method) {
        ...
        case 'percentage_risk':
          recommendedSize = this.calculatePercentageRisk(
            config.riskPercentage || 1,
            accountInfo.balance,
            stopLossDistance,
            priceInfo
          );
        ...
        case 'atr_based':
          if (config.atrBased) {
            recommendedSize = await this.calculateATRBased(
              config.atrBased,
              accountInfo,
              priceInfo
            );
        ...
        case 'volatility_based':
          if (config.volatilityBased) {
            recommendedSize = await this.calculateVolatilityBased(
              config.volatilityBased,
              priceInfo
            );
        ...
        case 'account_equity':
          recommendedSize = this.calculateEquityBased(
            accountInfo.equity,
            config.riskPercentage || 1,
            stopLossDistance
          );
        ...
        default:
          logger.warn(`Unknown sizing method: ${config.method}`);
      ...
      logger.error('[PositionSizingService] Error calculating position size:', error);
      throw error;
    }
  }
   ...
   */
  private async calculateATRBased(
    config: {
    ...
    if (atrValues.length === 0) {
      logger.warn('[PositionSizingService] No ATR values available');
      return 0.01;
    }
   ...
   */
  private calculateKellyCriterion(config: {
    winRate: number;
    avgWin: number;
    avgLoss: number;
