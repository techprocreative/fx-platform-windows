...
export class RegimeDetectionService {
   ...
   */
  async detectRegime(bars: Bar[]): Promise<RegimeDetectionResult> {
    
    if (bars.length < 200) {
      logger.warn('[RegimeDetectionService] Insufficient data for regime detection');
      return {
    ...
    logger.info(`[RegimeDetectionService] Detected regime: ${regime} (confidence: ${(confidence * 100).toFixed(1)}%)`);
  ...
  adaptStrategyForRegime(
    ...
    logger.info(`[RegimeDetectionService] Adapting strategy for ${regimeResult.regime}`);

    // Adjust position size
    if (regimeSettings.positionSizeMultiplier !== 1.0) {
      adapted.positionSize *= regimeSettings.positionSizeMultiplier;
      logger.info(`[RegimeDetectionService] Position size adjusted to ${adapted.positionSize} (${regimeSettings.positionSizeMultiplier}x)`);
    }
    ...
    if (adapted.takeProfit && regimeSettings.takeProfitMultiplier !== 1.0) {
      ...
      logger.info(`[RegimeDetectionService] Take profit adjusted (${regimeSettings.takeProfitMultiplier}x)`);
    }
    ...
    if (adapted.stopLoss && regimeSettings.stopLossMultiplier && regimeSettings.stopLossMultiplier !== 1.0) {
      ...
      logger.info(`[RegimeDetectionService] Stop loss adjusted (${regimeSettings.stopLossMultiplier}x)`);
    }
  ...
  isSuitableForTrading(
    ...
    if (!isAllowed) {
      logger.info(`[RegimeDetectionService] Current regime ${regimeResult.regime} not in allowed regimes`);
    }
   ...
   */
  getPositionSizeMultiplier(regime: MarketRegime): number {
    switch (regime) {
      case MarketRegime.BULLISH_TRENDING:
      case MarketRegime.BEARISH_TRENDING:
   ...
   */
  getTakeProfitMultiplier(regime: MarketRegime): number {
    switch (regime) {
      case MarketRegime.BULLISH_TRENDING:
      case MarketRegime.BEARISH_TRENDING:
  ...
  calculateTransitionProbability(
    ...
    const probabilities: Record<MarketRegime, number> = {
      [MarketRegime.BULLISH_TRENDING]: 0,
      [MarketRegime.BEARISH_TRENDING]: 0,
    ...
    switch (currentRegime) {
      case MarketRegime.RANGING:
        probabilities[MarketRegime.BREAKOUT] = indicators.adx > 15 ? 0.4 : 0.2;
        probabilities[MarketRegime.LOW_VOLATILITY] = 0.3;
        probabilities[MarketRegime.RANGING] = 0.3;
        break;

      case MarketRegime.BREAKOUT:
        probabilities[MarketRegime.BULLISH_TRENDING] = 0.35;
        probabilities[MarketRegime.BEARISH_TRENDING] = 0.35;
        probabilities[MarketRegime.RANGING] = 0.3;
        break;

      case MarketRegime.BULLISH_TRENDING:
      case MarketRegime.BEARISH_TRENDING:
        probabilities[MarketRegime.RANGING] = indicators.adx < 20 ? 0.4 : 0.2;
        probabilities[currentRegime] = 0.6;
        break;

      default:
        probabilities[MarketRegime.RANGING] = 0.5;
        probabilities[MarketRegime.BREAKOUT] = 0.3;
        probabilities[MarketRegime.HIGH_VOLATILITY] = 0.2;
    }
