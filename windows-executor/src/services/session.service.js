...
export class SessionService {
    constructor() {
        Object.defineProperty(this, "MARKET_SESSIONS", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: {
     ...
     */
    getCurrentSession(timezone = 'UTC') {
        const now = new Date();
     ...
     */
    analyzeSession(symbol, timezone = 'UTC') {
     ...
     */
    getOptimalPairsForCurrentSession() {
     ...
     */
    getAggressivenessMultiplier(symbol) {
        const analysis = this.analyzeSession(symbol);
     ...
     */
    adjustPositionSizeForSession(baseSize, symbol, useOptimalPairs = true) {
        if (!useOptimalPairs) {
            return {
     ...
     */
    getBestTradingTimesForPair(symbol) {
        const bestTimes = [];
        for (const [sessionName, session] of Object.entries(this.MARKET_SESSIONS)) {
     ...
     */
    shouldTradeInSession(symbol, allowedSessions = [], requireOptimalPair = false) {
        const currentSession = this.getCurrentSession();
        if (!currentSession) {
            return {
                shouldTrade: false,
