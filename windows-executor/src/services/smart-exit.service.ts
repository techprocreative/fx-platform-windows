...
export class SmartExitService {
   ...
   */
  async calculateSmartExit(
    config: SmartExitRules,
    context: ExitContext
  ): Promise<{
    shouldExit: boolean;
    newStopLoss?: number;
    ...
    if (config.stopLoss.useSwingPoints && config.stopLoss.swingLookback) {
      const swingLevel = this.findSwingPoint(
        bars,
        config.stopLoss.swingLookback,
        position.type
      );
   ...
   */
  async calculatePartialExits(
    config: EnhancedPartialExitConfig,
    context: ExitContext
  ): Promise<PartialExitCalculationResult> {
    ...
    const recommendedExits: PartialExitCalculationResult['recommendedExits'] = [];
  ...
  shouldLockInProfit(
    ...
    exitPercentage: number,
    config: EnhancedPartialExitConfig
  ): { shouldLock: boolean; newStopLoss?: number } {
