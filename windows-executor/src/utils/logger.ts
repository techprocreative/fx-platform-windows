...
const customFormat = winston.format.combine(
  ...
  winston.format.json(),
  winston.format.printf(({ timestamp, level, message, category, metadata, stack, ...meta }) => {
    let logObj: any = {
      timestamp,
...
export const createLogger = (category: string) => {
  return {
    ...
    // Security logging
    security: (message: string, severity: string, metadata?: any) => {
    ...
    // Trading logging
    trading: (message: string, metadata?: any) => {
      logger.info(message, { 
        category: 'trading', 
        metadata 
      });
...
export class PerformanceLogger {
  ...
  constructor(operation: string, category: string = 'performance', metadata?: any) {
